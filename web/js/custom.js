   ...
   jQuery('<div class="topbar"><a class="barmenu">'+
	...
	jQuery('.skin-color a').hover(function(){
		...
		if(jQuery('#skinstyle').length > 0) {
			if(s!='default') {
				jQuery('#skinstyle').attr('href','web/css/style.'+s+'.css');	
			...
			if(s!='default') {
				jQuery('head').append('<link id="skinstyle" rel="stylesheet" href="web/css/style.'+s+'.css" type="text/css" />');
				jQuery.cookie("skin-color", s, { path: '/' });
			}
		}
		return false;
	...
	if(jQuery.cookie('skin-color')) {
		...
		if(c) {
			jQuery('head').append('<link id="skinstyle" rel="stylesheet" href="web/css/style.'+c+'.css" type="text/css" />');
			jQuery.cookie("skin-color", c, { path: '/' });
		}
	}
	
	//récupérer les valeur dual select
	//jQuery('#selec_gestionnaire').change(function(){
	jQuery( "body" ).mousemove(function( event ) {

		//console.log(jQuery('#selec_gestionnaire').val());
		var values = ''
		jQuery('#selec_gestionnaire option').each(function() { values += jQuery(this).val()+'-'; });
		//console.log(values);
		jQuery("#gestionnaires").val(values);
	});
